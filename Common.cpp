...
bool toPublicKey(Secret const& _secret, unsigned _flags, array<CryptoPP::byte, KeySize>& o_serializedPubkey)
{
    auto* ctx = getCtx();
...
PublicCompressed dev::toPublicCompressed(Public const& _public)
{
	...
	std::array<CryptoPP::byte, 65> orgi;
	orgi[0] = 0x04;
...
Signature dev::sign(Secret const& _k, h256 const& _hash)
{
    ...
    ss.v = static_cast<CryptoPP::byte>(v);
    if (ss.s > c_secp256k1n / 2)
    {
        ss.v = static_cast<CryptoPP::byte>(ss.v ^ 1);
...
bool ecdh::agree(Secret const& _s, Public const& _r, Secret& o_s) noexcept
{
	...
	secp256k1_pubkey rawPubkey;
	std::array<CryptoPP::byte, 65> serializedPubKey{ { 0x04 } };
	...
	std::array<CryptoPP::byte, 32> compressedPoint;
...
bytes ecies::kdf(Secret const& _z, bytes const& _s1, unsigned kdByteLen)
{
	...
	std::array<CryptoPP::byte, 4> ctr{ { 0, 0, 0, 1 } };
	bytes k;
	...
	for (unsigned i = 0; i <= reps; i++)
	{
		...
		// append hash to k
		std::array<CryptoPP::byte, 32> digest;
